...
<div class="row px-4" id="contenedor_principal">
    <div class="col-12 card px-0">
        <div class="card-header mx-0">
            <h5 class="mb-0">Listado De historias clinicas </h5>
        </div>
        <div class="card-body">
                <!-- link de los tabs -->
                ...
                <table id="tbl-pacientes" class="table table-sm table-striped table-bordered">
                    <thead class="text-primary">
                        <th class="d-none d-sm-none d-md-table-cell">Id HC</th>
                        <th>Fecha de inicio</th>
                        <th>Identificación</th>
                        <th class="d-none d-sm-none d-md-table-cell">Paciente</th>
                        <th class="d-none d-sm-none d-md-table-cell">Medico</th>
...
<script>
    ...
    function listar(pagina){
        ...
        fetch(url, miInit).then(res => res.json()).catch(error =>  {
            ...
            if(resp.historias.length == 0){
                html = "<tr><td colspan='7'><b>No hay registros</b></td></tr>";
            }

            for(var i=0; i < resp.historias.length; i++){
                html+="<tr>";
                html+="<td>"+resp.historias[i].id+"</td>";
                html+="<td>"+resp.historias[i].historydate+"</td>";                
                html+="<td>("+resp.historias[i].tipodoc+") "+resp.historias[i].numdoc+"</td>";
                html+="<td>"+resp.historias[i].name1+" "+resp.historias[i].surname1+"</td>";
                html+="<td>"+resp.historias[i].name+"</td>";
                html+="<td>"+resp.historias[i].state+"</td>";
                html+="<td>";
                    html+="<a href='/apps/rol-medico/historias/ver/"+resp.historias[i].id+"' class='cl-azul mx-1'><i class='fas fa-eye'></i></a>";
                html+="</td>";
                html+="</tr>";
            }
