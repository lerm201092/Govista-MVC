            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="mat-div is-completed">
                        ...
                        <a style="font-size: 18px; float:right; cursor:pointer" title="Buscar Paciente" onclick="buscar_pacientes(1)"><span class="fa fa-search mr-1 cl-azul"></span></a>
                    </div>
                </div>
            </div> 
            <div class="row mt-3">
...
<script>
  ...
  $(".mat-input").focusout(function(){
    ...
    function buscar_pacientes(pagina){
        var texto = $("input[name=txt_buscar]").val(), texto = texto.trim(), texto = texto.toLowerCase();
        if(pagina == 1){
            $("#item-act").text(1);
        ...
        fetch(url, miInit).then(res => res.json()).catch(error =>  {
            ...
            for(var i=0; i < resp.pacientes.length; i++){
                ...
                html+="<tr>";
                    html+="<td id='tdname_"+id+"'>"+nombre+"</td>";
                    html+="<td id='tddoc_"+id+"'>"+doc+"</td>";
                    html+="<td class='text-center'><a href='javascript:sel("+id+")' class='px-2'><i class='fas fa-check'></i></a></td>";
                html+="</tr>";

            }

            if(resp.pacientes.length == 0){  html = "<tr><td colspan='3'>No hay registros</td></tr>";  }
    ...
    function guardar(){
        ...
        if(id_patient != ""){
            ...
            $("#p-alert-idpac").removeClass("d-none");
            location.href = "#p-alert-idpac";
            setTimeout(() => {
                $("#p-alert-idpac").addClass('d-none');    
            }, 5000);            
        }
    }
</script>
