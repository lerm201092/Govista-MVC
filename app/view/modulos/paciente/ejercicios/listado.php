...
<script>
    ...
    function listar(pagina){
        ...
        fetch(url, miInit).then(res => res.json()).catch(error =>  {
            ...
            if(resp.ejercicios.length == 0){
                html = "<tr><td colspan='7'><b>No hay registros</b></td></tr>";
            }

            for(var i=0; i < resp.ejercicios.length; i++){
                html+="<tr>";
                html+="<td>"+resp.ejercicios[i].id_historia+"</td>";
                html+="<td>"+resp.ejercicios[i].historydate+"</td>";                
                html+="<td>"+resp.ejercicios[i].name+"</td>";
                html+="<td>"+resp.ejercicios[i].observation+"</td>";
                html+="<td>"+resp.ejercicios[i].session+"</td>";
                html+="<td>"+(resp.ejercicios[i].session_ok ? resp.ejercicios[i].session_ok : 0 )+"</td>";
                html+="<td>"+resp.ejercicios[i].duration+"</td>";
                html+="<td>";
                    if( (parseInt(resp.ejercicios[i].session) - parseInt(resp.ejercicios[i].session_ok)) > 0){
                        html+="<a href='/apps/rol-paciente/ejercicios/play/"+resp.ejercicios[i].id+"/"+resp.ejercicios[i].id_exercise+"/"+resp.ejercicios[i].duration+"/"+resp.ejercicios[i].size+"/"+resp.ejercicios[i].id_historia+"' class='cl-verde mx-1'><i class='fas fa-play-circle'></i></a>";
                    }else{
                        html+="<span class='cl-rojo'><b>OK<b></span>";
                    }
                html+="</td>";
                html+="</tr>";
            }
